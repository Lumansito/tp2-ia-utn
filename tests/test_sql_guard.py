...
def test_insert_has_informational_warning():
    result = validate_and_classify_sql("INSERT INTO orders (id) VALUES (1)", dialect="postgres")
    assert result["classification"] == "WRITE"
    assert result["warning"] is not None
...
def test_sqlite_dialect_also_works():
    result = validate_and_classify_sql("SELECT * FROM customers", dialect="sqlite")
    assert result["classification"] == "SELECT"
    assert "LIMIT 500" in result["sanitized_sql"]
