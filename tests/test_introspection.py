...
def test_generate_single_table_ddl_includes_pk_and_fk():
    engine = _make_engine()
    meta = introspect_database(engine)
    ddl = generate_single_table_ddl("orders", meta["tables"]["orders"])
    assert "CREATE TABLE orders" in ddl
    assert "PRIMARY KEY" in ddl
    assert "FOREIGN KEY (customer_id) REFERENCES customers (id)" in ddl


def test_generate_ddl_preview_includes_all_tables():
    engine = _make_engine()
    meta = introspect_database(engine)
    ddl = generate_ddl_preview(meta)
